defmodule ElixirFastCharge.Stations.ChargingStationManager do
  ...
  defp wait_for_registry do
    case Process.whereis(ElixirFastCharge.Stations.StationRegistry) do
      nil ->
        Process.sleep(100)
        wait_for_registry()
      _pid ->
  ...
  def start_charging_station(station_id, station_data \\ %{}) do
    child_spec = %{
      id: station_id,
      start: {ElixirFastCharge.Stations.ChargingStation, :start_link, [station_id, station_data]},
      restart: :permanent,
      shutdown: 5000,
      type: :worker
    }
  ...
  def get_station_status(station_id) do
    ...
    ElixirFastCharge.Stations.ChargingStation.get_status(pid)
  end
